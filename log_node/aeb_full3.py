...
MIN_SAVE_INTERVAL_SEC  = 0.3   # 画像保存の最小間隔 (秒) [0.2s = 5fps程度]

class AEBImageAndSpeedLogger(Node):
    def __init__(self):
        super().__init__('aeb_image_speed_logger')
        ...
        self.last_image_save_time = None # ★追加: 最後に画像を保存した時刻
        ...
        self.get_logger().info(f"Save Interval: {MIN_SAVE_INTERVAL_SEC} sec (approx {1.0/MIN_SAVE_INTERVAL_SEC:.1f} fps)")
    ...
    def _on_image(self, msg: Image):
        ...
        if self.last_image_save_time is not None:
            ...
            if interval < MIN_SAVE_INTERVAL_SEC:
                return # まだ次の保存タイミングではないのでスキップ

        # 条件を満たしたので保存
        self._save_image(msg)
        self.last_image_save_time = now # 保存時刻を更新
    ...
    def _save_image(self, msg: Image):
        try:
            ...
            vel_info = f"vel={self.current_velocity:.2f}m/s" if self.current_velocity is not None else "vel=N/A"
